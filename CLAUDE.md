...
**rag_pipeline** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg)
, JWT + API Key auth, LangChain, RAG (qdrant)
...
# RAG
uv run rag_pipeline rag-collections
uv run rag_pipeline rag-ingest /path/to/file.pdf --collection docs
uv run rag_pipeline rag-search "query" --collection docs

# Sync Sources
uv run rag_pipeline cmd rag-sources
uv run rag_pipeline cmd rag-source-add
uv run rag_pipeline cmd rag-source-sync
```
