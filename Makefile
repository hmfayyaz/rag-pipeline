...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app rag_pipeline db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		rag_pipeline user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app rag_pipeline db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app rag_pipeline db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run rag_pipeline db migrate -m "initial" || true
	cd backend && uv run rag_pipeline db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend rag_pipeline db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend rag_pipeline db upgrade

db-downgrade:
	uv run --directory backend rag_pipeline db downgrade

db-current:
	uv run --directory backend rag_pipeline db current

db-history:
	uv run --directory backend rag_pipeline db history

# === Server ===
run:
	uv run --directory backend rag_pipeline server run --reload

run-prod:
	uv run --directory backend rag_pipeline server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend rag_pipeline server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend rag_pipeline user create-admin

user-create:
	uv run --directory backend rag_pipeline user create

user-list:
	uv run --directory backend rag_pipeline user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/rag_pipeline"
...
help:
	@echo ""
	@echo "rag_pipeline - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run rag_pipeline rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run rag_pipeline rag-search <query> -c <collection>  Search"
	@echo "  uv run rag_pipeline rag-collections                     List collections"
	@echo "  uv run rag_pipeline rag-sources                         List sync sources"
	@echo "  uv run rag_pipeline rag-source-add                      Add sync source"
	@echo "  uv run rag_pipeline rag-source-sync <id>                Trigger sync"
	@echo ""
