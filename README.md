...
└── commands/             # Click CLI commands (auto-discovered by `rag_pipeline cmd …`)
```
...
The generated project ships a Click CLI exposed as `rag_pipeline` (after `make install`):

```bash
rag_pipeline server run --reload          # dev server
rag_pipeline db upgrade                   # apply migrations
rag_pipeline db migrate -m "message"      # create new migration
rag_pipeline user create-admin            # interactive admin creation
rag_pipeline rag-ingest <path> -c docs    # ingest local files
rag_pipeline rag-search "query" -c docs   # semantic search
rag_pipeline rag-collections              # list collections
```

Run `make help` for a categorized list, or `rag_pipeline --help` for full CLI docs.
...
rag_pipeline rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
rag_pipeline rag-search "your query" --collection documents
```
