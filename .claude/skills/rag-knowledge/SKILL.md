...
```bash
uv run rag_pipeline cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run rag_pipeline cmd rag-search "your question" --collection docs        # semantic search
uv run rag_pipeline cmd rag-collections                                     # list collections
uv run rag_pipeline cmd rag-stats                                           # chunk/vector counts
uv run rag_pipeline cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run rag_pipeline cmd rag-sources                  # list
uv run rag_pipeline cmd rag-source-add               # add (interactive)
uv run rag_pipeline cmd rag-source-sync --all        # trigger a sync
```
